...
この章では、前提条件はない。
しかし、第I部 (基礎編) の練習問題を読んで挑戦し、第III部 (応用編) の章を参考に、ジオコンピューティングが仕事、研究、その他の問題の解決にどのように役立つかを考えていれば、より多くのものを得ることができるであろう。
...
作成されたクラス、属性、列の名称は `nz_u1` から `nz_u3` まで異なるが、その幾何学的な形状は同一であることを baseR 関数 `identical()` を使って検証している。^[
...
baseR の `aggregate()` 関数を使うか、**dplyr** の `summarise()` を使うかは好みの問題であるが、後者の方が読みやすいだろう。
...
ジオコンピュテーションをする際の選択で困ることと言えば、 **tidyverse**\index{tidyverse (package)} と baseR の間という、簡単に答えられない問題がある。
例えば、次のコードチャンクは、Chapter \@ref(attr) で説明したした `nz` オブジェクトから `Name` 列を抽出する操作を、**tidyverse** と baseR の２つの方法を示している。
...
答えは、「人それぞれ」である。
それぞれのアプローチには利点がある。baseR\index{R!base} は、安定でよく知られており、依存関係が最小である傾向があるため、ソフトウェア（パッケージ）開発に好まれることが多い。
...
地理データの操作、座標系の基礎、データの読み書き、可視化など、実際のアプリケーションで最も必要とされるトピックを重視するよう心がけることとした。
...
その他の話題としては、リモートセンシングや、GIS専用ソフトと並行してのRの利用（ブリッジとしてではなく）などが挙げられますが、これらは限定的なものでした。
...
[**Turf.js**](https://github.com/Turfjs/turf) は、JavaScriptでジオコンピュテーションを行う可能性を示す一例である。
\index{Scala}
\index{JavaScript}
...
再現性は、ジオコンピュテーションの初期の研究ではほとんど言及されていなかったが、最初の2つの要素に不可欠な要素であることを強く主張することができる。
...
さらに何十人もの人が、質問をしたり、**sf** が使っている「上流」のパッケージに貢献するのあどの方法で、貢献している。
...
この例は、数十のパッケージのうちのたった一つの例であるが、R を非常に効果的で継続的に進化する地理計算用言語とするために行われている知的作業の規模を表している。
