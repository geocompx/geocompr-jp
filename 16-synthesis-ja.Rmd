...
Chapter \@ref(intro) その中で私たちは、ジオコンピュテーションを、手法が一般にアクセス可能で、再現性\index{reproducibility}があり、協力的なコミュニティによってサポートされていることを保証する、より広い「オープンソースアプローチ」の一部として考えている。
...
これらのトピックは各章に繰り返し登場し、重複する部分が多いため、ジオコンピュテーションに不可欠なスキルを身につけることができる。 \index{geocomputation}。
...
最後に、ビッグデータ\index{big data}のジオコンピュテーションについては避けている。ここでビッグデータとは、個人 PC に保存できないデータセットや一つの CPU で処理しきれないデータセットを指す。
...
新しいジオコンピュテーションの方法論やアプローチを開発するときの最良の出発点は、しばしばペンと紙（または共同スケッチやアイデアの迅速な共有を可能にする [Excalidraw](https://excalidraw.com/) や [tldraw](https://www.tldraw.com/) などの同等のデジタルスケッチツール）である。方法論開発の作業ソフトウェアの最も創造的な初期段階においては、あらゆる種類の*は思考の速度を落とし、重要な抽象概念から思考を遠ざけることになる。
...
Chapter \@ref(intro) で紹介されているように、ジオコンピュテーションのための言語として R を学習することには十分な理由があるが、それが唯一の選択肢というわけではない。^[
私たちのジオコンピュテーションの定義に関連する R の強みは、科学的再現性の重視 \index{reproducibility}、学術研究での幅広い使用、地理データの統計的モデリングに対する比類のないサポートである。
さらに、コンテキストの切り替えにコストがかかるため、他の言語やフレームワークに手を出す前に、ジオコンピュテーション\index{geocomputation}のための1つの言語（R）を深く学ぶことを勧めている。
多くの言語の基礎知識を持つよりも、一つの言語の専門知識を持つことが望ましい。
]
*Python*\index{Python}、*C++*\index{C++}、*JavaScript*\index{JavaScript}、*Scala*\index{Scala}、*Rust*\index{Rust} を使っても、同じ深さでジオコンピュテーション勉強することができる。
それぞれが進化した地理空間能力を有している。
 [**rasterio**](https://github.com/mapbox/rasterio) は、例えば、Pythonのパッケージ
この本で使われている **raster** パッケージを補足/置換できるもの --- Python\index{Python} のエコシステムについては @garrard_geoprocessing_2016 や [automating-gis-processes](https://automating-gis-processes.github.io/CSC18) などのオンラインチュートリアルを参照。
...
Section \@ref(what-is-geocomputation) は、地理データ科学 (GDS) や「GIScience」など、ジオコンピュテーションとほぼ同じ意味を持つ他の用語を提示した。
 \index{data science}  どちらも地理データを扱うことの本質を捉えているが、ジオコンピュテーションには利点がある。本書で提唱する地理データの「計算」的な扱い方（コードで実装されているので再現性がある）を簡潔に捉え、初期の定義にあった望ましい要素に基づいて構築されている [@openshaw_geocomputation_2000]。
...
もし再現性がジオコンピュテーション（あるいはコマンドラインGIS）の決定的な資産であるならば、何が再現性をもたらすかを考える価値がある。
