...
しかし、以下のように、読み進めるにはとても厳しい前提条件がある。


- Part I (Foundations)の全章の練習問題を読み、挑戦したこと。
- 第 III 部 (応用編) では、ジオコンピュテーション\index{geocomputation}を使って、仕事やそれ以外の場面で、どのように実社会の問題を解決できるかを考えてみた。
...
R\index{R} の特徴として、同じ結果を得るために複数の方法が存在することが多い。
...
これは、baseR 関数 `identical()` を使って検証している。^[
...
よくある（そして時には論争の的になる）選択は、**tidyverse**\index{tidyverse (package)} と baseR のアプローチのどちらを取るかということである。
...
nz_name2 = nz |> select(Name)           # tidyverse による方法
identical(nz_name1$Name, nz_name2$Name) # 結果の確認
```

ここでまた、「どちらを使うか」という問題が出てくる。
答えは、「人それぞれ」である。
それぞれのアプローチには利点がある。パイプ構文は人気があり一部の人には魅力的であるが、base R\index{R!base} はより安定していて、多くの人に知られている。
...
R の部分集合演算子 `[` や **dplyr** 関数 `filter()` など、一般的に必要とされる演算子や関数について詳しく説明したが、他のパッケージの地理データを扱うための関数で、これまで言及されていないものが数多くある。
Chapter \@ref(intro) は、地理データを扱うための有力なパッケージを20以上紹介したが、この後の章ではそのうちの一部のパッケージについて紹介する。
実際には、このほかにも数百ものパッケージがある。
...
R の空間エコシステムの進化速度に圧倒されるだろうが、幅広い選択肢に対応するための戦略がある。
...
この多様性の中から、将来性があり（将来まで長く使える）、高性能（他の R パッケージと比較して）、補完性のあるパッケージに焦点を当てることとした。
しかし、例えば地図を作るためのパッケージの多様性に示されるように、私たちが使ってきたパッケージにはまだ重複がある（Chapter \@ref(adv-map) 参照）。

パッケージの重複は必ずしも悪いことではない。
...
2018年10月の **sf** に相当する数値は 69 であり、**sf** の人気が高まっていることから、今後も増加することが予想される。
]
...
このトピックに興味があれば、パッケージの[ソースコード](https://github.com/r-spatial/stars)やより広範な [SpatioTemporal Task View](https://cran.r-project.org/web/views/SpatioTemporal.html) で更新を確認することができる。
...
これらのトピックは各章に繰り返し登場し、重複する部分が多いため、ジオコンピュテーション\index{geocomputation}に不可欠なスキルを身につけることができる。 
...
本書は、空間統計モデリングや推論技術ではなく、機械学習\index{machine learning}に重点を置いている。
繰り返しになるが、これらのトピック、特に生態系のユースケースについては、@zuur_mixed_2009、@zuur_beginners_2017 や、David Rossiter による *Geostatistics &amp; Open-source Statistical Computing* の自由に使える教材やコードなどが既にあり、 [css.cornell.edu/faculty/dgr2](http://www.css.cornell.edu/faculty/dgr2/teach/) でホストされており、[*R for Geographic Data Science*](https://sdesabbata.github.io/r-for-geographic-data-science/) プロジェクトでは [レスター大学](https://le.ac.uk/people/stefano-de-sabbata) の [Stefano De Sabbata](https://stefanodesabbata.com/) による地理データ科学向けの R\index{R} の導入\index{data science} などがある。
また、モデリングと不確実性推定の強力なフレームワークであるベイズモデリングを使用した空間統計\index{spatial!statistics}に関する優れたリソースもある [@blangiardo_spatial_2015;@krainski_advanced_2018]。

最後に、ビッグデータ\index{big data}のジオコンピュテーションについては避けた。ここでビッグデータとは、個人 PC に保存できないデータセットや一つの CPU で処理しきれないデータセットを指す。
...
Chapter \ref(gis) で紹介した空間データベース\index{spatial database}は、メモリで処理しきれないデータセットの解析に有用である。
...
ジオコンピュテーションは大きな分野であり、目指すアウトカムに到達できない可能性が高い。
...
ジオコンピュテーションの定義に関連する R の強みは、科学的再現性の重視 \index{reproducibility}、学術研究での幅広い使用、地理データの統計的モデリングに対する比類のないサポートである。
さらに、他の言語やフレームワークを学ぶ際には時間も労力もかかるため、ジオコンピュテーション\index{geocomputation}のための1つの言語 (R) を深く学ぶことを勧めている。
多くの言語の基礎知識を持つよりも、一つの言語の専門知識を持つことが望ましい。
]
*Python*\index{Python}、*C++*\index{C++}、*JavaScript*\index{JavaScript}、*Scala*\index{Scala}、*Rust*\index{Rust} を使っても、同じ深さでジオコンピュテーションを勉強することができる。
それぞれが進化した地理空間能力を有している。
例えば、Pythonのパッケージ [**rasterio**](https://github.com/mapbox/rasterio) は、
この本で使われている **raster** パッケージを補足/置換できるものである。 Python\index{Python} のエコシステムについては @garrard_geoprocessing_2016 や [automating-gis-processes](https://automating-gis-processes.github.io/CSC18) などのオンラインチュートリアルを参照。
...
`r-spatial/sf` GitHub リポジトリをざっと見ると、**sf**\index{sf} パッケージの基礎となるコードをホストしており、40 人以上がコードベースとドキュメントに貢献していることがわかる。
さらに何十人もの人が、質問をしたり、**sf** が使っている「上流」のパッケージに貢献するのあどの方法で、貢献している。
その [issue tracker](https://github.com/r-spatial/sf/issues) には 600 以上の問題がクローズされ、より速く、より安定した、ユーザーフレンドリーな **sf** を実現するための膨大な作業が行われている。
