...
GIS ブリッジを通すことで、R は洗練された GIS 作業を実行できるにし、さらには CLI の持っている再現可能性、拡張性、生産性といった付加価値をもたらす。
...
knitr::kable(x = d, 
             caption = paste("３つのオープンソース GIS の比較。", 
                             "hybrid とは、ベクタとラスタ", 
                             "に対応していることを示す。"),
...
**qgisprocess**\index{qgisprocess (package)} パッケージは、QGIS コマンドラインユーティリティをラップ (訳注：プログラミング用語で、機能を使いやすく覆い隠すこと) しているため、R セッションから QGIS、GDAL、GRASS、SAGA アルゴリズムを呼び出すことが可能である。
...
```{r uniondata, echo=FALSE, fig.cap="二つの地域単位の例。不一致（黒線）と集約（赤色の境界）。"}
library(tmap)
...
`INPUT`、`OVERLAY`、`OVERLAY_FIELDS_PREFIX`、`OUTPUT`があることがすぐに分かる。
