...
この構文により、複数の形状やレイヤを持つマップを作成することができる。次のコードでは、関数 `tm_raster()` を使ってラスタレイヤ（レイヤを半透明にするために `col_alpha` が設定されている）を描画している様子を示している。
...
さらに、塗りつぶしと線の色の透過率を `fill_alpha` and  `col_alpha` で指定することあできる。
...
視覚化の変数には、`.scale`、`.legend`、`.free` という文字列を後ろのつけた３つの追加引数がある。
...
３番目のパレットである発散型パレットは、通常3色の間（紫-白-緑： Figure \@ref(fig:colpal) ）で、通常2つの単色パレットの両端を濃い色で結合して作成される。
...
map_nz + 
  ...
  tm_title("ニュージーランド") + tm_layout(fontfamily = "HiraginoSans-W3")
```
...
アスペクト比を得て、`viewport()` 関数を使い、二つの地図 (主地図とインセット地図) の大きさと位置を指定する。
...
インセット地図の表示領域は、大きさと位置を指定する必要がある。
...
最後に、新規にキャンバスを作り、主地図を表示し、主地図の領域内にインセット地図を配置する。

```{r insetmap1, message=FALSE, fig.cap="インセット地図で背景を説明 - ニュージーランドの南アルプスの中央部の位置。", fig.scap="Inset map providing a context."}
grid.newpage()
