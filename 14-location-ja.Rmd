...
```{r metro-areas, echo=FALSE, fig.width=1, fig.height=1, fig.cap="人口ラスタ（分解能20km）、大都市圏（金色のポリゴン）とその名称。", fig.scap="The aggregated population raster."}
...
もし `osm_id` カラムの代わりに `shop` カラムを使用した場合、グリッドセルあたりの取得店舗数は少なくなる。
これは、 `shop` カラムが `NA` 値を含んでいるためで、 `count()` 関数はベクトルオブジェクトをラスタライズする際にこの値を省略することができる。
```
