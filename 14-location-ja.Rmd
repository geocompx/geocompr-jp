...
この章では、パート I とパート II で学んだスキルを特定のドメインに適用する方法を示す。ジオマーケティング\index{geomarketing} （立地分析\index{location analysis}  やロケーションインテリジェンスとも呼ばれることがある）である。
研究・実用化されている分野は幅広い。
...
さらに `as.data.frame` を `TRUE` に設定すると、通りの名前、家の番号、都市名など、場所を示すいくつかのカラムを持つ `data.frame` が返される。
