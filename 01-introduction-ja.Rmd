...
好奇心旺盛な読者には、それらを試してみたり、本書の例を他の言語で再現してみたりすることをお勧めする。おそらく、[Geocomputation with Python](https://py.geocompx.org/)という本も参考になるだろう。そのオープンアクセス版は[geocompx.org](https://geocompx.org/) でもホストされている。
...
C++\index{C++} は、[QGIS](https://www.qgis.org/)\index{QGIS}、 [GRASS](https://grass.osgeo.org/)\index{GRASS}、[SAGA](https://saga-gis.sourceforge.io/)\index{SAGA} などの多くの GIS パッケージの基礎を提供しているので、賢明な出発点であると言えるだろう。
...
2023 年の引退は、これらのパッケージを適用している既存のワークフローに大きな影響を与えるだけでなく、これらのパケージに依存しているパッケージにも影響を与えた。 
