...
予想通り、空間交差検証の結果は、従来の交差検証アプローチよりも平均して低い AUROC 値をもたらし、後者の空間自己相関\index{autocorrelation!spatial}による楽観的な予測性能が強調された。
...
ハイパーパラメーター \index{hyperparameter}  パラメトリックモデルの係数と混同してはいけない。後者は、パラメータと呼ばれることもある（[machine mastery blog](https://machinelearningmastery.com/difference-between-a-parameter-and-a-hyperparameter/) も参照）。
係数はデータから推定できるが、ハイパーパラメータは学習開始前に設定する。
最適なハイパーパラメータは、通常、クロスバリデーション法を用いて定義された範囲内で決定される。
