...
航空宇宙、メディア、金融サービス、観光などの企業が集まり、2 つの主要な大学とともに、活気ある経済が展開されている。
...
先ほどのコードチャンクの結果、各ゾーンに 10 組以上の OD ペアがあることがわかった。つまり、下図のように、`bristol_zones`、結合する前に発着地データを集約する必要がある (発着地データは Section \@ref(desire-lines) に記述されている)。
...
この段階に至るまでには、(希望線から生成された)短いが車に依存する通勤経路を特定するための手順があり、(ルート・ネットワーク)セクションで **sfnetworks** パッケージを使用してルート・ネットワークの特性を分析することが含まれています。
