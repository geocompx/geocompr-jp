...
Section \@ref(crs-intro) は、座標参照系 (CRS) を紹介し、<u>地理座標系</u> ('lon/lat'、単位は経度と緯度) と<u>投影座標系</u> (通常は基準点からのメートル) という2つの主要なタイプに焦点を当てた。
...
2. `+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs` のような、形式化された、しかし今では時代遅れの「proj4 文字列」(または proj 文字列)
...
\index{raster!CRS}
CRS の取得と設定は、ラスタ地理データオブジェクトと同様の方法で行われる。
...
すべての CRS は、等面積、等距離、等角 (形状はそのまま)、またはそれらの妥協点の組み合わせである (「投影座標参照系」参照)。
