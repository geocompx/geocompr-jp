...
POINT フィーチャの属性\index{attribute}のうち、*name* のような名称の属性 (シンプルフィーチャの用語を使用する) が、この章のトピックである。
...
`sf` オブジェクトは、データフレーム用の `tidyverse` クラスである `tbl_df` と `tbl` を拡張することも可能である。\index{tidyverse (package)}
...
多くのアプリケーションにおいて、tidyverse\index{tidyverse (package)} パッケージ **dplyr** [@R-dplyr] は、データフレームを扱うための効果的なアプローチを提供する。
