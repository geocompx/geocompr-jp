...
```{r cropmask, echo = FALSE, fig.cap="ラスタクロップ、ラスタマスクの説明図。", fig.asp=0.36, fig.width = 10, warning=FALSE, dev="ragg_png"}
#| message: FALSE
#| results: hide
...
これは以下のコードで示されている。このコードは、ザイオン国立公園の標高値の要約統計を含むオブジェクト `zion_srtm_df` を作成する (Figure \@ref(fig:polyextr) (A) を参照)。
...
前述のコードチャンクは、Chapter \@ref(attr) で説明されているように、ポリゴン ID ごとのセル値の要約統計を提供するために **dplyr**\index{dplyr (package)} を使用した。
その結果、例えば公園の最高標高が海抜約 2,661 m であることなど、有用な要約が得られる (標準偏差など、他の要約統計もこの方法で計算できる)。
この例ではポリゴンが1つしかないので、1 行のデータフレームが返されるが、このメソッドは複数の範囲選択ポリゴンが使用されている場合に動作する。
...
```{r polyextr, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="連続 (左) とカテゴリ (右) のラスタ抽出に使用した範囲。", fig.width=7.5, dev="ragg_png"}
...
その最初の 2 つの引数は、`x`、ラスタ化されるベクタオブジェクトと、`y`、出力の範囲、解像度、CRS を定義する「テンプレートラスタ」オブジェクトである。
...
デフォルトでは、`fun = "last"` が使用されるが、`fun = "length"` などの他のオプションも使用できる。この場合、各グリッドセル内のサイクルレンタルの点の数をカウントする (この操作の結果は、Figure \@ref(fig:vector-rasterization1) (C) に示される)。
...
```{r contour-tmap, echo=FALSE, message=FALSE, fig.cap="モンゴル山南麓の等高線を重ねた陰影付き DEM。", fig.scap="DEM with hillshading.", warning=FALSE, fig.asp=0.56, fig.width=3.5}
...
```{r 06-raster-vector-40, echo=FALSE, fig.cap="ラスタ(左)のベクタ化、ポリゴン (dissolve = FALSE; 中央) と集約ポリゴン (dissolve = TRUE; 右)。", warning=FALSE, fig.asp=0.4, fig.scap="Illustration of vectorization."}
