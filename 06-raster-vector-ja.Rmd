...
\index{らすた・べくたへそうごさよう@ラスタ・ベクタ相互作用} 
...
この 2 つの処理により、オブジェクトのメモリ使用量と、その後の解析に必要な計算資源が削減される。ラスタデータを含む魅力的なマップを作成する前に必要な前処理工程となる場合がある。
...
\index{らすた@ラスタ!ちゅうしゅつわりあい@抽出 割合}
...
\index{くうかんべくたか@空間ベクタ化!contours}
...
```{r 06-raster-vector-40, echo=FALSE, fig.cap="ラスタ(左)のベクタ化、ポリゴン (dissolve = FALSE; 中央) と集約ポリゴン (dissolve = TRUE; 右)。", warning=FALSE, fig.asp=0.4, fig.scap="Illustration of vectorization.", dev="ragg_png"}
