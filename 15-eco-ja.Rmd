...
また、Chapters \@ref(gis) で解説した R\index{R} の専用 GIS\index{GIS} インタフェースや、Chapter \@ref(spatial-cv) の空間クロスバリデーション (cross-validation, CV)\index{cross-validation!spatial CV} も活用していこう。
...
そのために、これまでの章 (Chapter \@ref(spatial-class)〜\@ref(geometry-operations)、Chapter \@ref(gis)、Chapter \@ref(spatial-cv)) で紹介した概念をまとめ、さらに拡張していく。

霧のオアシスは、これまでに出会った中で最も魅力的な植生形態の一つである。このような現象は、地元では *lomas* と呼ばれ、ペルーやチリの海岸砂漠に沿った山々に発達している。^[同様の植生形態は、ナミビアやイエメン、オマーンの海岸沿いなど他の地域でも見られる [@galletti_land_2016]。] 
砂漠の極限状態と隔絶された地であることが、霧のオアシスの固有種を含むユニークな生態系の生息地となっている。
...
事例として、ペルー中央北岸のカスマ近郊にある *lomas* のある山、Mongón 山の南斜面における維管束植物の構成と空間分布を分析する (Figure \@ref(fig:study-area-mongon))。
