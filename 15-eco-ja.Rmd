...
また、、Chapters \@ref(gis) で解説した R\index{R} の専用 GIS\index{GIS} インタフェースや、Chapter \@ref(spatial-cv) の空間クロスバリデーション (cross-validation, CV)\index{cross-validation!spatial CV} も活用することができる。
...
`dem` は調査地域の数値標高モデル (digital elevation model, DEM\index{digital elevation model}) は Landsat シーンの赤色および近赤外チャネルから算出した正規化植生指標 (NDVI`ndvi`) である (Section \@ref(local-operations) および `?spDataLarge::ndvi.tif` を参照)。
