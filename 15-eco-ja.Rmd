...
`qgis_show_help()` 特定のジオアルゴリズム\index{geoalgorithm}のすべての関数\index{かんすう@関数}パラメータとデフォルト値を返す。
ここでは、その一部のみを紹介する。
...
1. 応答変数は数値なので、回帰\index{regression}タスクは、Section \@ref(svm) の分類\index{ぶんるい@分類}タスクに取って代わる。
